...
public class FinalReport implements SendableReport {
    ...
    public PriorityLevel getPriority() {
        ...
        } else if (!weirdEntryReports.isEmpty() || failedEntries > 0) {
    ...
    public String getEmailText() {
        String text =
                "Success: " + success + "\n\n" +
                        ...
                        "Added entries from us: " + addedEntriesFromUs + "\n" +
                        "Number of weird entries: " + weirdEntryReports.size() + "\n" +
                        "Number of unhandled exceptions: " + unhandledExceptions.size() + "\n\n";
        if (!weirdEntryReports.isEmpty()) {
            text += "Weird entries:\n";
            for (WeirdEntryReport weirdEntryReport : weirdEntryReports) {
                text += weirdEntryReport.getGisaidEpiIsl() + ": [" +
                        weirdEntryReport.getReportingFunction() + "] " +
                        weirdEntryReport.getMessage() + "\n";
            }
            text += "\n\n";
        }
    ...
    public FinalReport setImportMode(ImportMode importMode) {
    ...
    public FinalReport setAddedEntriesFromUs(int addedEntriesFromUs) {
    ...
    public FinalReport setWeirdEntryReports(List<WeirdEntryReport> weirdEntryReports) {
