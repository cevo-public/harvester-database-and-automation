  ...
  if (nrow(temp) > 0 & !all(is.na(temp$sample_number))) {
    print(notify.error(
      msg = paste(
        ...
        format_dataframe_for_log(temp %>% ungroup() %>% select(sample_name, ethid)),
        sep = "\n"
      ),
      fcn = paste0(args$script_name, "::", "qc_sample_metadata")
    ))
...
format_metadata_for_spsp <- function(metadata, args) {
  ...
  for (i in 1:nrow(metadata)) {
    if (metadata$covv_orig_lab[i] == "Institute of Medical Virology, University of Zurich") {
    ...
    if (metadata$sequencing_center[i] == "viollier") {
      metadata$authors[i] = paste(metadata$authors[i], seq_authors$viollier, sep = ", ")
    }
    if (metadata$sequencing_center[i] == "gfb") {
      metadata$authors[i] = paste(metadata$authors[i], seq_authors$gfb, sep = ", ")
    }
    if (metadata$sequencing_center[i] == "h2030") {
      metadata$authors[i] = paste(metadata$authors[i], seq_authors$h2030, sep = ", ")
    }
    if (metadata$sequencing_center[i] == "fgcz") {
      metadata$authors[i] = paste(metadata$authors[i], seq_authors$fgcz, sep = ", ")
    }
...
insert_additional_metadata <- function(metadata, args, db_connection) {
  ...
  for (i in 1:length(involved_labs)) {
    ...
    if (involved_labs[i] == "imv") {
      labs_meta[[i]] <- dplyr::tbl(db_connection, paste0(involved_labs[i], "_metadata")) %>%
