...
def import_sequences(
    ...
    if sample_names is None:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                f"SELECT sample_name FROM {DEST_TABLE} WHERE seq IS NOT NULL"
            )
    ...
    for sample_name in found_sample_names:
        ...
        seq_file = os.path.join(
            data_dir, sample_name, batch_to_import, "references/ref_majority_dels.fasta"
        )
            ...
            for record in seqs:
                ...
                try:
                    cursor.execute(
                        ...
                        " (sample_name, header, seq, sequencing_batch)"
                        " VALUES(%s, %s, %s, %s)",
                    ...
                    if update:
                        cursor.execute(
                            f"UPDATE {DEST_TABLE}"
                            " SET seq = %s, header = %s, sequencing_batch = %s"
                            " WHERE sample_name = %s",
                        ...
                        for table in (
                            "consensus_sequence_nextclade_mutation_aa",
                            "consensus_sequence_mutation_nucleotide",
                            "consensus_sequence_nextclade_data",
                        ):
                            cursor.execute(
                                f"DELETE FROM {table} WHERE sample_name = %s",
                                (sample_name,),
                            )
                        # Drop values in columns generated by
                        # import_sequence_diagnostic.R in consensus_sequence so
                        # diagnostic will be re-run on the updated sequences
                        cursor.execute(
                            f"UPDATE {DEST_TABLE}"
                            " SET divergence = NULL,"
                            "     excess_divergence = NULL,"
                            "     number_n = NULL,"
                            "     number_gaps = NULL,"
                            "     clusters = NULL,"
                            "     gaps = NULL,"
